jQuery(document).ready(function () {
    ...
    jQuery('[data-fancybox="galleries"]>img').each(function () {
        ...
        jQuery('[data-fancybox="galleries"]').eq(jQuery('[data-fancybox="galleries"]>img').index(this)).attr('href', src);
    })
    ...
    jQuery(".wpcf7-form input[type!='submit']").focusin(function () {
        jQuery(this).parents("p").addClass('input_focus');
    }).focusout(function () {
        jQuery(this).parents("p").removeClass('input_focus');
    });
    jQuery('.wpcf7-form input[type!="submit"],.wpcf7-form select').on('change', function () {
        var input = jQuery(this);
        var val = input.val();
        if (val != "")
            input.parents("p").addClass("value_focus");
        else
            input.parents("p").removeClass("value_focus");
    });
    ...
    jQuery("#epicwin_subscription input[type='text']").focusin(function () {
        jQuery(this).parents("p").addClass('input_focus');
    }).focusout(function () {
        jQuery(this).parents("p").removeClass('input_focus');
    });
    ...
    jQuery(".wpcf7-form input[value='Submit']").click(function (e) {
        ...
        if (email != "") {
            if (validateEmail(email)) {
                jQuery("#emailcustome").remove();
                jQuery(".wpcf7-form input[name='your-email']").removeClass("smart-error");
            } else {
                jQuery(".wpcf7-form input[name='your-email']").after(
                    '<div for="email" id="emailcustome" class="error-smart all-class">Please enter a valid email address.</div>'
                );
...
$(".down-button").click(function () {
    if (jQuery(window).width() <= 1024) {
        ...
        jQuery('html, body').animate({
            scrollTop: jQuery("#what-we-do").offset().top - jQuery('#header').height() + 200
        }, 800);
    }
});