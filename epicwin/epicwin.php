...
function epicwin_styles() {
	echo "<link rel='stylesheet' href='" . site_url() . "/wp-content/plugins/epicwin-subscribers/style.css' type='text/css' media='all' />";
}
	...
	<script type="text/javascript">
		...
		jQuery(document).on('submit','form#epicwin_subscription',function(e){
			e.preventDefault();
				...
				{
					jQuery('#subnewsemail').addClass('error');
					jQuery('.epic_results').html('<p class="error">Please enter a valid email address.</p>');
					setTimeout( function(){jQuery('.epic_results').html('');} , 10000);
					flag = true;	
				}
				if(flag==true)
				...
				jQuery.ajax({
					...
					success:function(results)
					{
					 ...
					 complete:function(results){
						...
						if(jQuery('.epic_results .errors p').hasClass('error')==true)
						{
						  jQuery('#subnewsemail').addClass('error');
						}
...
<form method="post" action="<?php echo currentUrl() ?>" class="epicwin-subscription" id="epicwin_subscription" >
		...
		<div class="email-form">
			...
			<div class="title"><p>Sign up to get the latest news, releases and more...</p></div>

			<div class="news-letter">

			<!--<div class="newsletter-email"><input placeholder="Name*" type="text" name="sub_name" class="news_text" id="subnewsname" /></div>-->

			<div class="newsletter-email"><label>Email Address<span>*</span></label><input type="text" name="sub_email" class="news_text" id="subnewsemail" /></div>

			

			<div class="email-subscribe"><div class="news-submit"><input  type="submit" id="sidebar_button" class="all-btn" value="SUBSCRIBE"/></div><div class="alax-loader"><img src="<?php echo get_site_url(); ?>/wp-content/uploads/2023/07/ajax-loader.gif" style="display:none;" class="loading" alt="loading"/></div></div></div>
...
<div class="epic_results"></div>
	...
	function check_info_details()
	{
		...
		if (filter_input(INPUT_POST, 'sub_email', FILTER_VALIDATE_EMAIL)) {
			...
			$errors[] = 'Vänligen skriv in ett giltigt e-postmeddelande';
		}
		...
		if ($errors) {
			...
			if($wpdb->insert('epicwin_feed', array('email' => $sub_email, 
			// 'name' => $sub_name, 
			'opt_in' => 1), array('%s', '%s', '%d'))) {
				$admin_email = get_option('admin_email');
				//$admin_email="siprogramming1@gmail.com";
				//~ echo $sub_email;
				//~ die();
				
				
				$to = $sub_email;
				$subject = "TRU-MED SOLUTIONS subscribed";
				$body = 'Hello,' . "\n";
				$body .= 'You have been subscribed to our newsletter.' . "\n";
				$body .= 'Regards,' . "\n";
				$body .= 'Team - TRU-MED SOLUTIONS.';
				//$body .= "<br />".home_url();
				// To send HTML mail, the Content-type header must be set
				$headers  = 'MIME-Version: 1.0' . "\r\n";
				$headers .= 'Content-type: text/plain' . "\r\n";
				$headers .= 'From:TRU-MED SOLUTIONS <'.$admin_email.">\r\n".
			...
			// for admin side email	
				$to = $admin_email;
				$subject = "New User Subscribed";
				$body = "New User Subscribed"."\n";
				$body .= "Email:- ".$sub_email;
				// To send HTML mail, the Content-type header must be set
				$headers  = 'MIME-Version: 1.0' . "\r\n";
				$headers .= 'Content-type: text/plain' . "\r\n";
				$headers .= 'From:TRU-MED SOLUTIONS <standard@webmasterindia.net>'.">\r\n".
				// 'Reply-To: '.$sub_email."\r\n" .
				'X-Mailer: PHP/' . phpversion();
